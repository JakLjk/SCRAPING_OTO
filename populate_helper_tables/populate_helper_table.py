...
from config import Config, OperationTypes
from logger import logger


wait_time_if_no_link_to_parse = Config.ETL.WAIT_IF_NO_OFFERS_TO_PARSE

from client_access_methods.populate_helper_table_client_methods import get_raw_data_for_helper_tables, get_currently_available_column_names, add_column, insert_value_in_db
...
def populate_helper_table():
    ...
    while True:
        ...
        data = get_raw_data_for_helper_tables()
        if data["Status"] == OperationTypes.status_failed:
            logger.warning(f"No data to be parsed. Waiting {wait_time_if_no_link_to_parse} seconds")
            sleep(wait_time_if_no_link_to_parse)
        foreign_offer_id = data["ID_O_P"]
        offer_details = data["offerDetails"]
        offer_equipment = data["equipmentDetails"]
        # if raw_data is None:
        ...
        current_columns = get_currently_available_column_names("separated_details_helper_table")
        ...
        for column in columns_to_add:
            add_column("separated_details_helper_table", column, "text")
        ...
        if not offer_details == "NO_INFORMATION" and not offer_equipment == '':
            ...
            logger.info("Fetching currently present columns in table")
            current_columns = get_currently_available_column_names("separated_equipment_helper_table")
            logger.info("Running comparison, to see which columns have to be added...")
            columns_to_add = set(separated_offer_equipment.keys()) - set(current_columns)
            logger.info(f"Adding {len(columns_to_add)} columns that were not present before.")
            for column in columns_to_add:
                add_column( "separated_equipment_helper_table", column, "BOOLEAN")
            logger.info(f"Missing columns added.")
            logger.info(f"Inseting values into database")
            insert_value_in_db( 
